...
const SalaryList = () => {
  const apiEndpoint = 'http://localhost:2000/hrm/sal/getAllSal';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/hrm/sal/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/hrm/sal/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
