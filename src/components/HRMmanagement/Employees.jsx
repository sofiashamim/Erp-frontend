...
const Employees = () => {
    ...
    const apiEndpoint = 'http://localhost:2000/hrm/emp/getAllEmp';
  ...
  const tableFields = [
    ...
    // { label: 'NID/passport', key: 'nidPassport' },
    // { label: 'NID/passport back', key: 'nidPassportBack' },
    {
      label: 'Status',
      key: 'status',
      render: (status) => (
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/hrm/emp/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      console.log(id)
      let response = await fetch(`http://localhost:2000/hrm/emp/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
 ...
 <button type="button" class="btn btn-outline-primary"><Link to="/employee">Employee list</Link></button>
 <button type="button"  class="btn btn-outline-primary"><Link to="/addemp">Add New Employee</Link></button>
          ...
          <div className="card">
            <div className="card-content">
              <div style={{ display: 'flex' }} className="card-body titleHead">
                ...
                >
                  <button type="button" className="btn btn-outline-primary"><Link to="/employee">Employee List</Link></button>
