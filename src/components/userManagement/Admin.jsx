...
const Admin = () => {
  const apiEndpoint = 'http://localhost:2000/user/admin/getAllAdmin';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/user/admin/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/user/admin/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
