...
const Merchandiser = () => {
    ...
    const apiEndpoint = 'http://localhost:2000/user/merch/getAllMerch';
    ...
    const onEdit = async (id) => {
      try {
        let response = await fetch(`http://localhost:2000/user/merch/update/${id}`, {
          method: 'PUT',
    ...
    const onDelete = async (id) => {
      try {
        let response = await fetch(`http://localhost:2000/user/merch/delete/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
  ...
  <button type="button" class="btn btn-outline-primary"><Link to="/addMerch">Add New Merchandiser</Link></button>
    ...
    <div className="row">
        <div style={{ marginLeft: '50px', marginTop: '100px' }} className="col-11">
          <div className="card">
            <div className="card-content">
              <div style={{ display: 'flex' }} className="card-body titleHead">
                <h4 className="card-title">MERCHANDISER LIST</h4>
                <div
                ...
                >
                  <button type="button" className="btn btn-outline-primary">
                    <Link to="/merchandiser">Merchandiser list</Link>
                  </button>
                  <button type="button" className="btn btn-outline-primary">
                    <Link to="/addMerch">Add New Merchandiser</Link>
                  </button>
                </div>
              </div>
              <hr />
