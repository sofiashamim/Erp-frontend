...
const Buyer = () => {
  ...
  const apiEndpoint = 'http://localhost:2000/user/buy/getAllBuyer';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/user/buy/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/user/buy/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
    ...
    <div className="row">
        <div style={{ marginLeft: '50px', marginTop: '100px' }} className="col-11">
          <div className="card">
            <div className="card-content">
              <div style={{ display: 'flex' }} className="card-body titleHead">
                <h4 className="card-title">BUYER LIST</h4>
                <div
                ...
                >
                  <button type="button" className="btn btn-outline-primary">
                    <Link to="/Buyer">Buyer list</Link>
                  </button>
                  <button type="button" className="btn btn-outline-primary">
                    <Link to="/addBuyer">Add New Buyer</Link>
                  </button>
                </div>
              </div>
              <hr />
