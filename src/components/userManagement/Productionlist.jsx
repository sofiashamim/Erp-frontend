...
const ProductionList = () => {
  const apiEndpoint = 'http://localhost:2000/user/prod/getAllProduction';
  ...
  const onEdit = async (id) => {
    try {
      const response = await fetch(`http://localhost:2000/user/prod/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:2000/user/prod/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
