...
const CreateAccessory = ({ onUnitCreated }) => {
  ...
  useEffect(() => {
    ...
    const fetchAccessoryList = async () => {
      try {
        let response = await fetch('http://localhost:2000/inv/accList/getAllAccessory');
  ...
  const handleAccessSubmit = async (e) => {
    ...
    let url = id
      ? `http://localhost:2000/inv/accList/update/${id}`
      : 'http://localhost:2000/inv/accList/create';
