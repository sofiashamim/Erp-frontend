...
const AccorderList = () => {
  const apiEndpoint = 'http://localhost:2000/inv/accOrderList/getAllAccOrder';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/inv/accOrderList/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/inv/accOrderList/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
