...
const AccessoryList = () => {
  const apiEndpoint = 'http://localhost:2000/inv/accList/getAllAccessory';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/inv/accList/update/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      console.log(id)
      let response = await fetch(`http://localhost:2000/inv/accList/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
