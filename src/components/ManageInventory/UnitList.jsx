...
const UnitModule = () => {
  const apiEndpoint = 'http://localhost:2000/api/unit/getAllUnit';
  ...
  const onEdit = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/api/unit/edit/${id}`, {
        method: 'PUT',
  ...
  const onDelete = async (id) => {
    try {
      let response = await fetch(`http://localhost:2000/api/unit/delete/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
