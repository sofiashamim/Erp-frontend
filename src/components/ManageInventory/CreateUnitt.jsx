...
//     let res = await fetch('http://localhost:2000/api/unit/create', {
//       method: 'POST',
//       headers: {
...
const CreateUnit = ({ onUnitCreated }) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      const fetchUnitList = async () => {
        try {
          let response = await fetch('http://localhost:2000/api/unit/getAllUnit');
  ...
  const handleUnitSubmit = async (e) => {
    ...
    let url = id
      ? `http://localhost:2000/api/unit/edit/${id}`
      : 'http://localhost:2000/api/unit/create';
