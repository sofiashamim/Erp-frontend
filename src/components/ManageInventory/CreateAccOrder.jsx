...
const CreateAccOrder = ({ onUnitCreated }) => {
  ...
  useEffect(() => {
    ...
    const fetchAccessoryList = async () => {
      try {
        let response = await fetch('http://localhost:2000/inv/accOrderList/getAllAccOrder');
  ...
  const handleAccessSubmit = async (e) => {
    ...
    let url = id
      ? `http://localhost:2000/inv/accOrderList/update/${id}`
      : 'http://localhost:2000/inv/accOrderList/create';
